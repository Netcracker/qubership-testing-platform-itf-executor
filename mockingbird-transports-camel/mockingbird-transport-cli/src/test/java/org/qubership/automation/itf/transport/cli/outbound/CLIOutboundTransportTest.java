...
public class CLIOutboundTransportTest {
            ...
            try {
                ...
                while (!Thread.interrupted()) {
                    try {
                        ...
                        LoggerFactory.getLogger(CLIOutboundTransportTest.class).error("Failed read data from socket",
                                e);
                    }
                }
            } catch (Exception e) {
                LoggerFactory.getLogger(CLIOutboundTransportTest.class).warn("Something went wrong, check it.", e);
            }
        });
    }
    ...
    private String resolveEndpoint(ConnectionProperties properties) throws IllegalArgumentException, IOException {
        ...
        return isSsh ?
                "ssh:"
                        + properties.get(PropertyConstants.Cli.USER)
                        + ((passIsBlank)
                        ? "@"
                        : ':' + password.toString() + "@")
